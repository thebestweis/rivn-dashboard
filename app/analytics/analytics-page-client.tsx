...
import {
  PlanFactTab,
  type PlanFactRow,
} from "../components/analytics/plan-fact-tab";
...
export default function AnalyticsPage() {
  ...
  const [payrollAccruals, setPayrollAccruals] = useState<StoredPayrollAccrual[]>([]);
  ...
  const totalFotNumber = useMemo(() => {
    const payoutsSum = payrollPayouts
      .filter((p) => {
  ...
  }, [payrollPayouts, extraPayments, selectedMonth]);
  ...
  }, [clients, allPaymentRecords, expenses, payrollAccruals]);
  ...
  const revenueDynamics = useMemo(() => {
    ...
    extraPayments.forEach((item) => {
      ...
      const month = toSupabaseLikeDate(item.date).slice(0, 7);
      if (!month) return;

      const current = map.get(month) || { revenue: 0, expenses: 0, fot: 0 };
      current.fot += parseRubAmount(String(item.amount ?? ""));
      map.set(month, current);
    });
  ...
  }, [payments, expenses, payrollPayouts, extraPayments]);
  ...
  const growthMetrics = useMemo(() => {
    ...
    const newClients = Math.max(0, baseClients + growthScenario.clientsDelta);
  ...
  const planFactChartData = useMemo(() => {
    ...
    revenueDynamics.forEach((item) => {
      ...
      const monthPayoutsFot = payrollPayouts
      ...
      const monthExtraFot = extraPayments
        .filter((extra) => toSupabaseLikeDate(extra.date).startsWith(item.month))
        .reduce(
          (sum, extra) => sum + parseRubAmount(String(extra.amount ?? "")),
          0
        );
      ...
      factMap.set(item.month, {
        ...
        fot: monthPayoutsFot + monthExtraFot,
      });
    });
