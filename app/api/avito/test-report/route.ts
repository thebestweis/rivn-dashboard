...
function buildMetricLine(label: string, current: number, previous: number, type: "number" | "money" | "percent") {
  const change = getChangePercent(current, previous);

  let formattedValue = "";

  if (type === "money") {
    formattedValue = formatMoney(current);
  }

  if (type === "percent") {
    formattedValue = formatPercentValue(current);
  }
  ...
  try {
    ...
    if (!clientCode) {
      return Response.json(
        {
          ok: false,
          error: "Передай clientCode для точечного теста",
        },
        { status: 400 }
      );
    }
    ...
    if (clientError || !client) {
      return Response.json(
        {
          ok: false,
          error: clientError?.message || "Активный клиент с Telegram chat_id не найден",
        },
        { status: 404 }
      );
    }
    ...
    if (accountsError || !accounts || accounts.length === 0) {
      return Response.json(
        {
          ok: false,
          error: accountsError?.message || "Активные Avito-аккаунты клиента не найдены",
        },
        { status: 404 }
      );
    }
      ...
      try {
        ...
        try {
          if (
            ...
            preparedCurrent.qualityStatus !== "critical"
          ) {
            currentStatsRaw = {
    ...
    const reportText = [
      "🧪 <b>Тестовый Avito-отчёт RIVN OS</b>",
      "",
      buildDailyReport({
    ...
    return Response.json({
      ok: true,
      message: "Мультиаккаунтный тестовый отчёт отправлен в Telegram",
      client: client.name,
