...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as InvoiceCreatedBody;

    if (!body.paymentId) {
      return NextResponse.json({ error: "Не передан paymentId" }, { status: 400 });
    }
