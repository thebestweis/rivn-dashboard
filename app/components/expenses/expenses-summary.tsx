  ...
  return (
    <section className="grid gap-3 md:grid-cols-3 md:gap-4">
      {cards.map((item) => (
        <div
          key={item.label}
          className="rounded-[24px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))] p-4 shadow-[0_10px_40px_rgba(0,0,0,0.28)] sm:p-5"
        >
          <div className="text-sm text-white/55">{item.label}</div>
          <div className={`mt-3 text-2xl font-semibold tracking-tight sm:text-3xl ${item.tone}`}>
            {item.value}
          </div>
        </div>
      ))}
